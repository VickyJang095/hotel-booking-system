...
@section('title', 'Hoàn tất đặt phòng - Tripto')
...
$payLaterDate = $ciObj->copy()->subDays(10)->format('d/m');
$ratingLabel = ($hotel->rating ?? 5) >= 4.5 ? __('search.excellent') : __('search.very_good');
$currency = app(\App\Services\CurrencyService::class);
@endphp
<form method="POST" action="{{ route('booking.store') }}" onsubmit="return confirmPayment(event)">
    @csrf
    ...
    <input type="hidden" name="guest_name" value="{{ request('guests')[0]['first_name'] ?? '' }} {{ request('guests')[0]['last_name'] ?? '' }}">
    <input type="hidden" name="guest_email" value="{{ request('guests')[0]['email'] ?? '' }}">
    <input type="hidden" name="guest_phone" value="{{ request('guests')[0]['phone'] ?? '' }}">
        ...
        <div class="max-w-2xl mx-auto px-4 py-5">
            <div class="flex items-center gap-0">
                <div class="flex flex-col items-center gap-1.5 shrink-0">
                    ...
                    <span class="text-xs font-medium text-gray-400 whitespace-nowrap">{{ __('booking.step_selection') }}</span>
                </div>
                ...
                <div class="flex flex-col items-center gap-1.5 shrink-0">
                    ...
                    <span class="text-xs font-medium text-gray-400 whitespace-nowrap">{{ __('booking.step_details') }}</span>
                </div>
                <div class="step-line done mx-2 mb-5"></div>
                <div class="flex flex-col items-center gap-1.5 shrink-0">
                    <div class="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center text-sm font-bold">3</div>
                    <span class="text-xs font-bold text-gray-900 whitespace-nowrap">{{ __('booking.finish_booking') }}</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="flex gap-6 items-start">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-5">
                        <h2 class="text-xl font-bold text-gray-900 mb-4">{{ __('booking.choose_when_to_pay') }}</h2>
                        <div class="space-y-3">
                            <label class="pay-option selected flex items-start gap-3 cursor-pointer" id="opt-now-label">
                                <input type="radio" name="pay_when" value="now" checked onchange="selectPayOption('now')" class="mt-0.5 shrink-0">
                                <div>
                                    <p class="text-sm font-bold text-gray-900">{{ __('booking.pay_now_label', ['amount' => $currency->formatPrice($grandTotal)]) }}</p>
                                </div>
                            </label>
                            <label class="pay-option flex items-start gap-3 cursor-pointer" id="opt-split-label">
                                <input type="radio" name="pay_when" value="split" onchange="selectPayOption('split')" class="mt-0.5 shrink-0">
                                <div>
                                    <p class="text-sm font-bold text-gray-900">{{ __('booking.pay_split') }}</p>
                                    <p class="text-sm text-gray-500 mt-0.5">
                                        {{ __('booking.pay_split_desc', ['now' => $currency->formatPrice($halfTotal), 'later' => $currency->formatPrice($grandTotal - $halfTotal), 'date' => $payLaterDate]) }}
                                        <a href="#" class="text-blue-600 font-semibold hover:underline">{{ __('booking.learn_more') }}</a>
                                    </p>
                                </div>
                            </label>
                            <label class="pay-option flex items-start gap-3 cursor-pointer" id="opt-klarna-label">
                                <input type="radio" name="pay_when" value="klarna" onchange="selectPayOption('klarna')" class="mt-0.5 shrink-0">
                                <div>
                                    <p class="text-sm font-bold text-gray-900">{{ __('booking.pay_klarna') }}</p>
                                    <p class="text-sm text-gray-500 mt-0.5">
                                        {{ __('booking.pay_klarna_desc', ['amount' => $currency->formatPrice($installment)]) }}
                                        <a href="#" class="text-blue-600 font-semibold hover:underline">{{ __('booking.learn_more') }}</a>
                                    </p>
                                </div>
                            </label>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 p-6 mb-5">
                        <h2 class="text-xl font-bold text-gray-900 mb-5">{{ __('booking.complete_payment') }}</h2>

                        <h3 class="text-sm font-bold text-gray-800 mb-3">{{ __('booking.personal_details') }}</h3>
                        <div class="grid grid-cols-2 gap-3 mb-3">
                            <div>
                                <label class="text-xs font-semibold text-gray-500 mb-1.5 block">{{ __('booking.address_line') }}</label>
                                <input type="text" name="address" placeholder="Số nhà, tên đường" class="form-input">
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-500 mb-1.5 block">{{ __('booking.city') }}</label>
                                <input type="text" name="city" placeholder="Hà Nội" class="form-input">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-3 mb-6">
                            <div>
                                <label class="text-xs font-semibold text-gray-500 mb-1.5 block">{{ __('booking.postal_code') }}</label>
                                <input type="text" name="postal_code" placeholder="100000" class="form-input">
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-500 mb-1.5 block">{{ __('booking.province') }}</label>
                                <input type="text" name="state" placeholder="Hà Nội" class="form-input">
                            </div>
                        </div>

                        <h3 class="text-sm font-bold text-gray-800 mb-3">{{ __('booking.payment_methods') }}</h3>
                        <div class="flex gap-2 flex-wrap mb-6">
                            <button type="button" onclick="selectPM(this,'visa')" class="pm-pill active" data-pm="visa">
                                <span class="font-black text-blue-700 text-base tracking-tight">VISA</span>
                            </button>
                            <button type="button" onclick="selectPM(this,'diners')" class="pm-pill" data-pm="diners">
                            ...
                            <button type="button" onclick="selectPM(this,'mastercard')" class="pm-pill" data-pm="mastercard">
                            ...
                            <button type="button" onclick="selectPM(this,'stripe')" class="pm-pill" data-pm="stripe">
                                <span class="font-black text-indigo-600 text-base tracking-widest">stripe</span>
                            </button>
                            <button type="button" onclick="selectPM(this,'paypal')" class="pm-pill" data-pm="paypal">
                                <span class="font-black text-[#003087] text-base">Pay<span class="text-[#009cde]">Pal</span></span>
                            </button>
                            <button type="button" onclick="selectPM(this,'gpay')" class="pm-pill" data-pm="gpay">
                            ...
                            <button type="button" onclick="selectPM(this,'applepay')" class="pm-pill" data-pm="applepay">
                        ...
                        <h3 class="text-sm font-bold text-gray-800 mb-3">{{ __('booking.card_details') }}</h3>
                        <div class="space-y-3" id="cardDetails">
                            <div>
                                <label class="text-xs font-semibold text-gray-500 mb-1.5 block">{{ __('booking.cardholder_name') }} <span class="text-red-400">*</span></label>
                                <input type="text" name="cardholder_name" placeholder="{{ __('booking.cardholder_ph') }}" class="form-input">
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-gray-500 mb-1.5 block">{{ __('booking.card_number') }} <span class="text-red-400">*</span></label>
                                <div class="card-input-wrap">
                                    <input type="text" name="card_number" placeholder="{{ __('booking.card_number_ph') }}"
                                        maxlength="19" oninput="formatCard(this)" class="form-input pr-20">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="text-xs font-semibold text-gray-500 mb-1.5 block">{{ __('booking.card_expiry') }} <span class="text-red-400">*</span></label>
                                    <input type="text" name="expiry" placeholder="MM/YY" maxlength="5" oninput="formatExpiry(this)" class="form-input">
                                </div>
                                <div>
                                    <label class="text-xs font-semibold text-gray-500 mb-1.5 block">CVC <span class="text-red-400">*</span></label>
                                    <input type="text" name="cvc" placeholder="CVC" maxlength="4" oninput="this.value=this.value.replace(/\D/g,'')" class="form-input">
                                </div>
                            </div>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="save_card" value="1" checked>
                                <span class="text-sm text-gray-700 font-medium">{{ __('booking.save_card') }}</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="summary-card">
                            ...
                            <div class="flex gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-1.5 mt-1">
                                        ...
                                        <span class="text-xs text-gray-400">{{ number_format($hotel->review_count ?? 1260) }} {{ __('search.reviews') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="p-4 border-b border-gray-100">
                            ...
                            <div class="mt-3 pt-3 border-t border-gray-100">
                                <p class="text-xs font-semibold text-gray-500 mb-0.5">{{ __('booking.rooms_and_guests') }}</p>
                                <p class="text-sm text-gray-700">{{ $rooms }} {{ __('booking.room_unit') }}, {{ $adults }} {{ __('booking.adult_unit') }}</p>
                            </div>
                        </div>
                        <div class="p-4">
...
<script>
    ...
    function confirmPayment(e) {
        ...
        if (cardMethods.includes(pm)) {
            if (!cardHolder || !cardNum || !expiry || !cvc) {
