...
@section('page-title', 'Edit My Hotel')
@section('page-subtitle', 'Update your hotel information')

@section('content')
<div class="max-w-3xl">
    <form method="POST" action="{{ route('owner.hotel.update') }}"
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="total_rooms" min="1"
                           value="{{ old('total_rooms', $hotel->total_rooms) }}" class="form-input" required>
                </div>
                <div>
                    ...
                    <input type="number" name="max_guests_per_room" min="1"
                           value="{{ old('max_guests_per_room', $hotel->max_guests_per_room) }}" class="form-input" required>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div class="mb-3">
                <img src="{{ $hotel->image_url }}" alt="Current"
                     class="h-32 rounded-xl object-cover"
                     onerror="this.style.display='none'">
            </div>
            <div class="space-y-3">
                <div>
                    <label class="text-xs font-semibold text-gray-500 mb-1.5 block">Upload New Image</label>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div class="flex flex-wrap gap-5">
                @foreach(['free_cancellation'=>'Free Cancellation','instant_booking'=>'Instant Booking','pay_at_property'=>'Pay at Property'] as $field => $label)
                <label class="flex items-center gap-2 cursor-pointer">
